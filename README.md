...
- [Cấu hình](#cấu-hình)
- [Testing (Testing)](#kiểm-thử-testing)
...
- **Shopping Cart** (Cart) - Hỗ trợ Cart cho khách (không cần đăng nhập) và tự động gộp (merge) khi Authentication
- **User Accounts** (Tài khoản người dùng) - Đăng ký/đăng nhập an toàn với Authentication JWT
...
- **JWT Authentication** - Authentication API Security dựa trên token
- **Authorization** - Kiểm soát truy cập dựa trên vai trò (Role-based access control: Admin/Customer)
- **Data Validation** - Authentication đầu vào tại tầng DTO với DataAnnotations
...
- **Unit Tests** - Hơn 16 unit tests toàn diện bao phủ Authentication (authentication), Cart (cart), và các thao tác mật khẩu
...
- **DTO Pattern** - Data Transfer Objects để Authentication request/response của API
- **Soft Delete** - Xóa logic bằng cách đánh dấu `IsDeleted = true` thay vì xóa cứng (hard deletion)
- **Session-based Carts** - Cart tạm thời cho khách thông qua HttpOnly cookies
- **JWT Tokens** - Authentication không trạng thái (stateless authentication) với tự động gộp Cart khi login
...
- **Note:** Trả về Cart của guest hoặc user dựa trên trạng thái authentication
...
- PasswordHelperTests.cs - 6 tests cho hashing/verification (băm và Authentication) mật khẩu
- CartServiceTests.cs - 3 tests cho các thao tác với Cart
- AuthControllerTests.cs - 5 tests cho luồng Authentication
...
│   │   ├── Controllers/
│   │   │   ├── AuthController.cs        # Các endpoint Authentication User
...
## 🔐 Security Best Practices (Các Mẫu Security Tốt)
...
- **Authorization:** Kiểm tra Authorization sử dụng attribute `[Authorize(Roles="Admin")]`
...
- **[Tóm tắt Dự án (Project Summary)](docs/project-summary-vi.md)** - Tổng quan về dự án
- **[Tóm tắt Cuối cùng (Final Summary)](docs/final-summary.md)** - Danh sách kiểm tra đầy đủ các tính năng đã triển khai
- **[Kiểm toán Kiến trúc (Architecture Review)](docs/architecture-review.md)** - Phân tích chi tiết về kiến trúc hệ thống
- **[Kiểm toán Bảo mật (Security Review)](docs/security-review.md)** - Đánh giá toàn diện về bảo mật và authentication
- **[Hướng dẫn Giỏ hàng (Cart Guides)](docs/cart-optimization-guide.md)** - Hướng dẫn tối ưu hóa và sử dụng giỏ hàng
  - [PUT/DELETE Giỏ hàng](docs/cart-put-delete-guide.md) - Chi tiết về các endpoint PUT/DELETE
- **[Hướng dẫn Checkout](docs/checkout-api-guide.md)** - API checkout hoàn chỉnh
  - [Hướng dẫn Test Checkout](docs/checkout-test-guide.md) - Kịch bản test chi tiết
...
**Cập nhật lần cuối:** 02 Tháng 04, 2026  
